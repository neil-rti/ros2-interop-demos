...
/*ci
 * \brief cros2illuminance class constructor
 *
 * \details
 * Instantiate a sensor_msgs::msg::dds_::Illuminance_ publisher 
 * and/or subscriber with supporting writer, reader, topic, etc.
 *
 * \param[in]    illuminanceName    Name of the DDS topic
 * \param[in]    opt_en       bitfield of options (pub, sub) to enable
 * \param[in]    participant  DDS participant, already created
 */
cros2illuminance::cros2illuminance(const std::string illuminanceName, uint32_t opt_en, dds::domain::DomainParticipant participant,
    int(*rcv_fptr)(dds::sub::DataReader<sensor_msgs::msg::dds_::Illuminance_>)) :
    data_topic(dds::core::null), pub_sample(),
    data_wr(dds::core::null), data_rd(dds::core::null),
    waitset()
{
    // create topic (pub or sub)
    const std::string data_topname("rt/" + illuminanceName);
    data_topic = dds::topic::Topic<sensor_msgs::msg::dds_::Illuminance_>(participant, data_topname);

    if (opt_en & (1 << CROS2_PUB_ON)) {
        // DataWriter
        data_wr = dds::pub::DataWriter<sensor_msgs::msg::dds_::Illuminance_>(dds::pub::Publisher(participant), data_topic);
    }

    if (opt_en & (1 << CROS2_SUB_ON)) {
        // DataReader, ReadCondition, WaitSet
        data_rd = dds::sub::DataReader<sensor_msgs::msg::dds_::Illuminance_>(dds::sub::Subscriber(participant), data_topic);
...
int cros2illuminance::rcv_dataProc(dds::sub::DataReader<sensor_msgs::msg::dds_::Illuminance_>& reader)
{
    int count = 0;
    dds::sub::LoanedSamples<sensor_msgs::msg::dds_::Illuminance_> samples = reader.take();
...
/*ci
 * \brief Publish the Illuminance_ topic.
 *
 * \details
 * Timestamp and publish the Illuminance_ topic
 *
 */
void cros2illuminance::publish(void)
{ 
    timespec ts;
    tstamp_get(&ts);
...
// destructor
cros2illuminance::~cros2illuminance() {}
