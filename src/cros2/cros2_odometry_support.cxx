/** ==================================================================
 * FILE: cros2_odometry_support.cxx
...
#include "cros2_odometry_support.hpp"
 ...
 /*ci
  * \brief cros2odometry class constructor
  *
  * \details
  * Instantiate a nav_msgs::msg::dds_::Odometry_ publisher
  * and/or subscriber with supporting writer, reader, topic, etc.
  *
  * \param[in]    odometryName  Name of the DDS topic
  * \param[in]    opt_en        bitfield of options (pub, sub) to enable
  * \param[in]    participant   DDS participant, already created
  */
cros2odometry::cros2odometry(const std::string odometryName, uint32_t opt_en, dds::domain::DomainParticipant participant,
    int(*rcv_fptr)(dds::sub::DataReader<nav_msgs::msg::dds_::Odometry_>)) :
    data_topic(dds::core::null), pub_sample(),
    data_wr(dds::core::null), data_rd(dds::core::null),
    waitset()
{
    // create topic (pub or sub)
    const std::string data_topname("rt/" + odometryName);
    data_topic = dds::topic::Topic<nav_msgs::msg::dds_::Odometry_>(participant, data_topname);

    if (opt_en & (1 << CROS2_PUB_ON)) {
        // DataWriter
        data_wr = dds::pub::DataWriter<nav_msgs::msg::dds_::Odometry_>(dds::pub::Publisher(participant), data_topic);
    }

    if (opt_en & (1 << CROS2_SUB_ON)) {
        // DataReader, ReadCondition, WaitSet
        data_rd = dds::sub::DataReader<nav_msgs::msg::dds_::Odometry_>(dds::sub::Subscriber(participant), data_topic);
...
int cros2odometry::rcv_dataProc(dds::sub::DataReader<nav_msgs::msg::dds_::Odometry_>& reader)
{
    int count = 0;
    dds::sub::LoanedSamples<nav_msgs::msg::dds_::Odometry_> samples = reader.take();
...
/*ci
 * \brief Publish the Odometry_ topic.
 *
 * \details
 * Timestamp and publish the Odometry_ topic
 *
 */
void cros2odometry::publish(void)
{ 
    timespec ts;
    tstamp_get(&ts);
